...
class TransactionGenerator:
    ...
    def generate_transaction(self):
        transaction_payload = 'This is a transaction between A and B. ' \
                              'We add a random seed here {} to make its hash unique'.format(
                                  self.random_seed)
...
class Block:
    ...
    def apply_mining_step(self):
        ...
        print('CURRENT_BLOCK_HASH = {}, TARGET = {}'.format(
            current_block_hash, self.target))
...
class BlockChain:
    ...
    def notify_everybody(self):
        ...
        print('[block #{}] : {}'.format(
            len(self.block_chain), self.get_last_block()))
        print('-' * 80)
...
def my_first_miner():
    ...
    for i, block_added in enumerate(block_chain.block_chain):
        print('Block #{} was added. It took {} steps to find it.'.format(
            i, block_added.nounce))
